...
from app_prog.models import User, Announcement, Local, Public, Session
from app_prog.validation_schemas import announcement_schema
...
def create_announcement():
    ...
    announcement = Announcement(tittle=data['tittle'],
...
def get_list():
    ...
    for ann in announcement_list:
        ann_json.append({'tittle': ann.tittle,
                         'content': ann.content,
...
def get_announcement(announementId):
    ...
    public = session.query(Public).filter_by(announcementid=announementId).first()
    if not public:
...
def update_announcement(announementId):
    ...
    if 'tittle' in data.keys():
        announcement.tittle = data['tittle']
...
def delete_announcement(announcementId):
    ...
    if (announcement.authorid != user.id):
        return Response(status=401, response='You are not allowed to delete this announcement')
    if local:
        session.delete(local)
    else:
